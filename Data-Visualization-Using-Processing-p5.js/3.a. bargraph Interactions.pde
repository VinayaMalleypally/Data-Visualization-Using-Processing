...
void mouseMoved(){
  ...
  if(mouseX>50 && mouseX<=50+(table1.getRowCount()*2.75) && get(mouseX,mouseY)!=-1381654)
  {     
    fill(255,20,10);
    stroke(0,0,200);
    for(int i=0;i<table1.getColumnCount();i++)
    {
      count[i]=table1.getFloat(int(floor((mouseX-50)/2.75)),i);
      fill(0);
      textSize(13);
      strokeWeight(4);
  ...
  if(keyPressed)
  {
    ...
    {                            
     ...
     for(int k=0;k<table1.getRowCount();k++)
     {
      ...
      fill(0,0,255);
      stroke(5);
      rect(50+(2.75*k),height-50,2.75,-data.getFloat(pick)*var);          
     }
    if(mousePressed)
    {  
        mousePressed();  
